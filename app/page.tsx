  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const [pedidosRes, ventasRes, cobroRes, prospectoRes, productosRes, muestrasRes] = await Promise.all([
  ...
  return (
    <div className="p-4 md:p-6 pb-20 md:pb-6 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {[
          { label: 'Pedidos Pendientes', value: stats?.pedidosPendientes ?? 0, color: '#ff9800', suffix: '' },
          { label: 'Ventas Hoy', value: fmt(stats?.ventasHoy ?? 0), color: '#4caf50', suffix: '' },
          { label: 'Por Cobrar', value: fmt(stats?.totalPorCobrar ?? 0), color: '#e53935', suffix: '' },
          { label: 'Prospectos', value: stats?.prospectosPorContactar ?? 0, color: '#2196f3', suffix: '' },
        ].map((s) => (
      ...
      <div
        ...
        style={{ backgroundColor: '#141414', borderColor: '#ff9800' + '40', borderLeftWidth: 4, borderLeftColor: '#ff9800' }}
      >
